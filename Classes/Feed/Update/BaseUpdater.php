...
abstract class BaseUpdater implements FeedUpdaterInterface
{
    ...
    /**
     * Makes requests to download feed images
     *
     * @var RequestFactory
     */
    ...
    public function mimetypeToExt($mime) {
        $mime_map = [
            'video/3gpp2'                                                               => '3g2',
            'video/3gp'                                                                 => '3gp',
            'video/3gpp'                                                                => '3gp',
    ...
    public function createImageRelations(Feed $feed)
    {
        try {
            ...
            if (empty($fileObjects) && ($feed->getSmallImage() !== "")) {
    ...
    public function storeImg($url, BaseUpdater $instance, Feed $feeditem)
    {
        ...
        $folderNormal = 'socialfeed/'.$feeditem->getType()."/";
        if (!$storage->hasFolder($folderNormal)) {
            $storage->createFolder($folderNormal);
        }
        $downloadFolder = $storage->getFolder($folderNormal);
        $hashedFilename = md5($url);

        try {
            $response = $this->requestFactory->request($url, 'GET', ['sink' => '/tmp/temporary_file_name.ext']);
            ...
            if ($response->getStatusCode() === 200) {
                ...
                $feeditem->setSmallImage((string)$newFile->getUid());
        ...
        return [
            'normal_image' => '/' . $conf['basePath'] . 'socialmedia/' . $hashedFilename,
            //'small_image' => '/' . $conf['basePath'] . 'socialmedia/instacontent/small/MHGEI_' . $hashedFilename
